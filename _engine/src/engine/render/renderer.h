...
class NYRenderer 
{
	...
	public :
		...
		void render(float elapsed)
		{
			...
			if(!_transition && _DeltaTimeCumul >= 15.0)
			{
				_startTime = 15.0;
				_transition = true;
			}
			...
			if(_DoPostProcess)
			{
				...
				var = glGetUniformLocation(_ProgramPP, "iGlobalTime");
				glUniform1f(var, _DeltaTimeCumul);

				var = glGetUniformLocation(_ProgramPP, "screen_height");
				glUniform1f(var, (float)_ScreenHeight);

				var = glGetUniformLocation(_ProgramPP, "transition");
				glUniform1f(var, (bool)_transition);

				var = glGetUniformLocation(_ProgramPP, "startTime");
				glUniform1f(var, (float)_startTime);
