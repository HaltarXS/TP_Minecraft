...
IABase(_world), m_cone(m_viewAngle, m_viewDistance)
{
	...
	type = WASTEDOSAURE;//Spécification du type
...
//Permet de spécifier l'ensemble des entités du monde 
void Wastedosaure::SetEntities(std::map<eTypeCreature, CreatureVector> * entities)
{
	m_entities = entities;
}

//Récupère l'ensemble des créatures attaquables dans le champ de vision.
...
//Renvoie le chemin de l'entité
Path Wastedosaure::GetPath()
{
	return m_path;
}

//Renvoie vrai si l'entité a un chemin qu'elle peut suivre.
bool Wastedosaure::HasAPath()
{
	return m_path.GetSize() > 0;
}

//Renvoie le type de cube sur lequel se trouve une entité.
NYCubeType Wastedosaure::GetCubeUnderType(IABase * target)
{
	//On récupère la bonne position et on renvoie le type du cube à la position trouvée.
...
//Renvoie le type de cube à une position X,Y donnée.
...
//Trouve le cube d'eau le plus proche de l'entité.
...
void Wastedosaure::UpdateIA()
{
	...
	if (m_currentState != STATE_Dead)
	{
		...
		//Si le leader est abscent, on va faire un AssignToAGroup pour désigner le nouveau leader et réorganiser le groupe
		if (leader != NULL && leader->m_currentState == STATE_Dead)
		{
			WastedosaureManager::GetSingleton()->AssignToAGroup(this);
		}

		//Si On a une créature dans notre champ de vision
		if (m_creaturesInSight.size() > 0 &&
			...
			GetCubeUnderType(m_creaturesInSight[0]) != NYCubeType::CUBE_EAU) //Et que le créature que l'on veut attaquer ne se situe pas sur de l'eau.
		{
			//cout << "Creature in sight\n";
			WastedosaureManager::GetSingleton()->PrepareAttack(m_creaturesInSight[0]); //On prépare une attaque
		}

		//On met à jour la taille du Wastedosaure
...
void Wastedosaure::UpdateTimers()
{
	...
	{
		if (m_timerWandering >= m_durationWandering)//On met à jour les timers
		{
...
bool Wastedosaure::States(StateMachineEvent event, MSG_Object * msg, int state)
{
	...
	partner = NULL;		 //On set le partner à null au cas où
	...
	{
		PushState(STATE_FindPath);//Quand on a fini d'etre un oeuf, on va chercher un chemin pour se déplacer.
	}
	m_timerEgg += m_lastUpdate.getElapsedSeconds();
	OnExit
		WastedosaureManager::GetSingleton()->AssignToAGroup(this); //Une fois que je sors du mode oeuf, on assigne l'entité au groupe
	//On assigne son sexe
	...
	if (GetCubeUnderType(arrival) == NYCubeType::CUBE_EAU)//Si le cube d'arrivé es un cube d'eau, on sait que l'on ne va pas trouver de chemin...
	{
		PushState(STATE_FindPath);//...alors on retourne dans le même state sans chercher de chemin pour éviter des freeze du au parcours de toutes les nodes pour trouver un chemin.
	}
	else
	{
		...
		m_currentIndex = FindClosestWaypoint(m_path);//On trouve l'index le plus pret pour éviter de revenir au tout début du chemin pour le suivre. Ca ferait bizarre.
		if (HasAPath())//Si on a bien un chemin.
		{
			...
			if (distanceClosestWP.getSize() > 20.0f)//Si le closest point est trop loin, on lance quand meme un pf vers le leader pour éviter que l'entité passe à travers les murs le plus possible, même si il y a des ratés des fois :(.
			{
	...
	if (HasAPath())
	{
		PushState(STATE_Move);//Si on a un chemin, va passer dans l'état Move pour le suivre
	}
	...
	{
		partner->PushState(STATE_Move); //Il se met à bouger aussi
	}
	OnUpdate
	...
	//Si on a tué la cible, on va se reproduire 
	...
	{
		//On va se préparer à trouver un endroit pour se reproduire
		m_path.Clear();
		m_timerWandering = 0.0f;
		m_timerReproduction = 0.0f;
		m_currentIndex = 0;
		...
		//On va trouver un endroit pour se reproduire à proximité des autres wastedosaures.
	...
	else
	{
		...
		if (m_canReproduce && //Que l'on peut se reproduire
			...
			offsetPartners1.getSize() <= 1 && //Que l'entité est arrivé au point de reproduction
			offsetPartners2.getSize() <= 1 &&//et que son partenaire aussi
			(*m_entities)[WASTEDOSAURE].size() < MAX_WASTEDOSAURE //Qu'il n'y a pas déjà trop de wastedosaures
			/*&& m_isMale != partner->m_isMale //et qu'ils ont le même sexe*/)
		{
			//On incrémentes les compteurs de reproduction
	...
	m_currentIndex = 0;
	target = NULL; //Au cas où
	partner = NULL;//Au cas où
	OnUpdate
		//Si on a un chemin, on va jusqu'à l'eau
	if (HasAPath())
	{
		...
		{
			PushState(STATE_Dead);//Quand on est arrivé, l'entité meurt
		}
	}
	else
	{
		PushState(STATE_Dead); //Si on a pas trouvé de chemin, on meurt sur place
	}
